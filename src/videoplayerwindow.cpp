...
VideoPlayerWindow::VideoPlayerWindow(QWidget *parent) : QWidget(parent) {
  setWindowTitle("IPTV++");
...
void VideoPlayerWindow::handlePlaybackError(const QString &error) {
  ...
  if (m_errorLabel) {
    ...
    m_errorLabel->setText(
        QString("Error de conexión/reproducción: %1").arg(error));
...
void VideoPlayerWindow::setupUi() {
  ...
  m_loadingSpinner->setStyleSheet("QProgressBar {"
  ...
  m_errorLabel->setStyleSheet("QLabel {"
                              "   color: #ff5555;"
  ...
  m_controlsWidget = new QWidget(m_videoWidget);
...
void VideoPlayerWindow::setupConnections() {
  connect(m_player, &MpvPlayer::bufferingChanged, this,
          &VideoPlayerWindow::onBufferingChanged);

  connect(m_player, &MpvPlayer::playbackError, this,
          &VideoPlayerWindow::handlePlaybackError);

  connect(m_player, &MpvPlayer::positionChanged, this,
          &VideoPlayerWindow::updatePosition);

  connect(m_player, &MpvPlayer::durationChanged, this,
...
void VideoPlayerWindow::resizeEvent(QResizeEvent *event) {
  ...
  if (m_controlsWidget && m_videoWidget) {
    int controlsHeight = m_controlsWidget->height();
    int parentWidth = m_videoWidget->width();
    int parentHeight = m_videoWidget->height();

    m_controlsWidget->setGeometry(0, parentHeight - controlsHeight, parentWidth,
                                  controlsHeight);
...
void VideoPlayerWindow::play(const QString &url) {
  ...
  if (m_loadingSpinner) {
    m_loadingSpinner->show();
    m_loadingSpinner
        ->raise(); // Asegura que se renderice sobre la superficie de mpv
  }
